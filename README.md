...
План MVP — [docs/00-PLAN.md](docs/00-PLAN.md). Поточний стан: **задача 05** (мертві зони:
тунелі та виїмки з OSM + ручний файл спостережень; зони видно на стрічці й на карті).
...
- На першій відчутній мертвій зоні (`severity: none`, ≥ 1 км; якщо таких немає — на 40%
  маршруту, 3 км) фікси зникають → статус `немає GPS`, км заморожений. Так перевіряється
  поведінка в тунелі.
...
src/
  ...
  components/   Map.tsx — єдине місце роботи з MapLibre; RouteRibbon, TripStatusBar,
                TripCard, OperatorPicker, Toast, ZoneSheet (деталі мертвої зони)
